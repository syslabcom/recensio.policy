...
class XMLRepresentation(BrowserView):
    ...
    def list_authors(self):
        ...
        for author in self.context.getAuthors():
            out += AUTHOR_TMPL % dict(num=num, firstname=author['firstname'], lastname=author['lastname'])
            num += 1
        return out
    ...
    def list_editors(self):
        ...
        for editor in self.context.getEditorial():
            out += EDITOR_TMPL % dict(num=num, firstname=editor['firstname'], lastname=editor['lastname'])
            num += 1
        return out
...
class XMLRepresentation_publication(XMLRepresentation):
    ...
    def reviews(self):
        ...
        path = dict(query='/'.join(self.context.getPhysicalPath()), depth=3)
