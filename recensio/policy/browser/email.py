        ...
        for result in self.context.new_discussions.queryCatalog():
            ...
            line = u'%s (%s %s)\n' \
                % (result.getObject().getDecoratedTitle(),
                   result.total_comments, label_comments)
            line += '\n'
            retval += u'''<p>%s<br>
(<a href="%s">%s</a>)</p>
''' \
    ...
    def getNewPresentations(self):
        ...
        formatted_result = lambda x: \
            u'<p>%s<br>(<a href="%s">%s</a>)</p>\n' \
            % (x.getObject().getDecoratedTitle(), x.getURL(),
               x.getURL())
...
'''
    ...
    def getMailAddresses(self):
        ...
        mail_to = '%s <%s>' % (mail_info.email_from_name,
                               mail_info.email_from_address)
    ...
    def __call__(self):
        ...
        try:
            ...
            sections = {}
            sections['new_review_sections'] = \
                self.getNewReviewSections()
            ...
            try:
                arch = \
                    self.context.unrestrictedTraverse(settings.archive_folder)
            except (AttributeError, KeyError):

                # try to create archive folder

                folder = getToolByName(self.context, 'portal_url'
                        ).getPortalObject()
                for sub in settings.archive_folder.split('/')[2:]:
                    if sub in folder.objectIds():
                        ...
                        if not (type == 'ATFolder' or type
                                == 'Plone Site'):
                            messages.addStatusMessage('Unable to create archive folder %s: %s is not a folder!'
                                     % (settings.archive_folder,
                                    folder.getId()), type='error')
                            break
                        else:
                            ...
                            folder.setTitle(sub)
                arch = folder
                messages.addStatusMessage('Created Newsletter archive folder %s'
                         % settings.archive_folder, type='info')

            if not getattr(arch, 'meta_type', None) == 'ATFolder':
                messages.addStatusMessage('Unable to use %s as archive folder: Not a folder!'
                         % settings.archive_folder, type='error')
                raise ValidationError

            if not arch.getPhysicalPath() \
                == tuple(settings.archive_folder.split('/')):
                raise ValidationError('Invalid archive folder path')

            name = 'Newsletter %s' % DateTime().strftime('%d.%m.%Y')
            if name in arch.objectIds():
                messages.addStatusMessage('%s already exists in archive'
                         % name, type='warning')
            else:
                ...
                messages.addStatusMessage('Mail archived as %s'
                        % '/'.join(new_ob.getPhysicalPath()),
                        type='info')
            ...
            if self.errors:
                ...
                messages.addStatusMessage(self.ts.translate(_('mail_sending_prepared'
                        ,
                        default='Mailversand wird vorbereitet. Mail wird versandt an %(mail_to)s'
                        ), context=self.context,
                        mapping={u'mail_to': mail_to}), type='info')
...
''' \
    ...
    def findRecipient(self, id=None):
        membership_tool = getToolByName(self.context,
                'portal_membership')
        owner = membership_tool.getMemberById(id
                or self.context.__parent__.__parent__.Creator()).getUser()
...
class MailNewPublication(BrowserView):
    ...
    def __init__(self, request, context):
        ...
        self.membership_tool = getToolByName(self.context,
                'portal_membership')
...
class MailUncommented(BrowserView):
    ...
    def __init__(self, request, context):
        ...
        self.mailhost = getToolByName(self.context, 'MailHost')
        self.mail_body = \
            u'''Sehr geehrte/r Frau/Herr ${name},
...
Sie haben am ${date} Ihre Schrift
    ...
    def __call__(self):
        count = 0
        for result in \
            self.context.discussion_three_months_old.queryCatalog():
    ...
    def sendMail(self, result):
        ...
        subject = self.ts.translate(_('mail_uncommented_subject',
    ...
    def formatMessage(self, result):
        ...
        msg = self.ts.translate(_('mail_uncommented_body',
