...
def getSelectedQuery(selected, params=None, facet_fields=browsing_facets, queryparam='fq'):
    ...
    for field, values in selected.items():
        for name in values:
            if name:
                if queryparam == 'fq':
                    params.setdefault(queryparam, []).append('%s:"%s"' % (field, name.encode('utf-8')))
                else:
                    params.setdefault(queryparam, []).append(name.encode('utf-8'))
    return make_query(params, doseq=True)


def convertFacets(fields, context=None, request={}, filter=None, facet_fields=browsing_facets, queryparam='fq'):
        ...
        for name, count in sorted(values.items(), cmp=second):
            ...
            if filter is None or filter(name, count):
                counts.append(dict(name=name, count=count,
                    query=getSelectedQuery({field: [name]}, params=p, facet_fields=facet_fields, queryparam=queryparam)))
        if counts:
