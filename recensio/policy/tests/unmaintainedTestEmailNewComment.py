...
Ihr recensio.net-Team"""
        ...
        reviews = [
            portal["sample-reviews"]["newspapera"]["summer"]["issue-2"].objectValues()[
                0
            ],
        ...
        membership_tool.addMember(
            "commenter",
            "12345",
            [],
            [],
            properties={"email": "jc@example.org", "preferred_language": "de"},
        )
        membership_tool.addMember(
            "commenter2",
            "12345",
            [],
            [],
            properties={"email": "hc@example.org", "preferred_language": "de"},
        )
        ...
        for review in reviews:
            ...
            view()
            self.assertEquals(
                len(view.mailhost.sentMail), len(expected_mail[review.portal_type])
            )
            for i in range(len(view.mailhost.sentMail)):
                for lineno, (expected, real) in enumerate(
                    zip(
                        expected_mail[review.portal_type][i].split("\n"),
